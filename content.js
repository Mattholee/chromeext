...
chatContainer.setAttribute('style', 'width: 85%; height: 30%; position: fixed; bottom:16px; right: 16px;')
document.body.appendChild(chatContainer);

const chatInterface = document.createElement('div');
chatInterface.setAttribute('id', 'chatInterface')
chatInterface.setAttribute('style', 'width: 80%; height: 30%; position: fixed; bottom:16px; right: 16px; background-Color: white; border: 1px solid black; border-radius: 10px 10px 10px 10px; overflow: scroll; font-size: 32px;')
...
chatHistory.setAttribute('style', 'width: 80%; height: 100%; font-size: 20px; padding: 8px');
document.querySelector('#chatInterface').appendChild(chatHistory);

const toggleButton = document.createElement('button')
toggleButton.innerText = 'Toggle';
toggleButton.setAttribute('id', 'toggleButton')
toggleButton.setAttribute('style', 'width: 75px; height: 25px; position:absolute; top:0; right:25px;')
...
chatGPTButton.setAttribute('style', 'width: 100px; height: 50px; position: absolute; bottom:25px; right: 25px')
chatGPTButton.addEventListener('click',() => {
    ...
    if (selectedText.length === 0) {
        document.getElementById('chatQuestion').innerText = ('How to highlight text on your browser:'); 
        document.querySelector('#chatHistory').innerHTML = 'Thinking...'
        queryChatGPT('How to highlight a text on your browser');
    }
    else {
        document.getElementById('chatQuestion').innerText = ('Explain ' + selectedText); 
        document.querySelector('#chatHistory').innerHTML = 'Thinking...'
...
examplesButton.setAttribute('style', 'width: 100px; height: 50px; position: absolute; bottom:75px; right: 25px')
examplesButton.addEventListener('click',() => {
    ...
    if (selectedText.length === 0) {
        ...
        document.getElementById('chatQuestion').innerText = ('Here are examples of ' + selectedText); 
        document.querySelector('#chatHistory').innerHTML = 'Thinking...'
...
piratefy.setAttribute('style', 'width: 100px; height: 50px; position: absolute; bottom:125px; right: 25px')
piratefy.addEventListener('click',() => {
    ...
    document.getElementById('chatInterface').style.display = "block";
    if (selectedText.length === 0) {
        document.getElementById('chatQuestion').innerText = ('Arrr, ye lookin\' to be highlightin\' some text on yer browser?'); 
        document.querySelector('#chatHistory').innerHTML = 'Ye\'ll need to select the text ye want highlighted and then press CTRL+C to copy it and then press CTRL+V to paste it. That\'ll do the trick!'
        //queryChatGPT('How to highlight a text on your browser');
    }
    else {
        document.getElementById('chatQuestion').innerText = ('How a pirate would explain ' + selectedText); 
        document.querySelector('#chatHistory').innerHTML = 'Thinking...'
